import {
  ...
  export function useMonitor() {
    ...
    const footer = shallowRef (
      defineAsyncComponent(() => import('../components/FooterInferiorXs.vue')),
    );
    ...
    const updateBreakpoint = () => {
      ...
      if (width < 576) {
        ...
        breakpoint.value = 'xs';
        menu.value = defineAsyncComponent(() =>
          import('../components/FooterInferiorXs.vue'),
        );
      } else if (width < 1201) {
        breakpoint.value = 'lg';
        menu.value = defineAsyncComponent(() =>
          import('../components/FooterInferiorLg.vue'),
        ); }
        else if (width < 768) {
        breakpoint.value = 'sm';
        menu.value = defineAsyncComponent(() =>
